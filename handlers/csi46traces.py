...
class CSI46Traces(Handler):

    FS = 50
    ALL_ACTIVITIES = ["clapping", "falling", "jumping", "nothing", "walking", "waving"]
    ...
    def loadByActivityLabel(self, directory, activity, activities=ALL_ACTIVITIES, saveToFile=False, windowSize=FS, step=6, downsample=1):
        ...
        datDataPathPattern = os.path.join(directory, "dat", "*", "*_{}_*.dat".format(activity))
    ...
    def loadDataForActivities(self, directory, activities=ALL_ACTIVITIES, saveToFile=False, windowSize=50, step=25, downsample=1):
