...
class WiAR(Handler):
    ...
    FS = 30
    ALL_ACTIVITES = ["horizontal_arm_wave", "high_arm_wave", "two_hands_wave", "high_throw", "draw_x", "draw_tick", "toss_paper", "forward_kick", "side_kick", "bend", "hand_clap", "walk", "phone_call", "drink_water", "sit_down", "squat"]
    ...
    def loadFromDat(self, inputFile, windowSize=FS, step=30):
        ...
        for entry in csi_trace:
            ...
            ind = 0
            for x in range(30):
                reshapedData[ind] = csiData[x][0]
                reshapedData[ind+1] = csiData[x][1]
                reshapedData[ind+2] = csiData[x][2]
                ind += 3
    ...
    def loadByActivityLabel(self, directory, activity, activities=ALL_ACTIVITES, saveToFile=False, windowSize=FS, step=6, downsample=1):
        ...
        activityNo = self.ALL_ACTIVITES.index(activity)+1
        datDataPathPattern = os.path.join(directory, "data", "*", "csi_a{}_*.dat".format(activityNo))
    ...
    def loadDataForActivities(self, directory, activities=ALL_ACTIVITES, saveToFile=False, windowSize=120, step=30, downsample=1):
...
if __name__ == "__main__":
    print("Running WiAR tests.")

    directory = "E:\\Datasets\\WiAR"
    config = {
        "downsample": 1
    }

    test = WiAR(directory, config)
